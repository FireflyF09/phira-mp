...
async function loadRooms(){
  try{
    const r=await fetch('/admin/api/rooms');const d=await r.json();
...
async function loadBans(){
  try{
    const r=await fetch('/admin/api/bans');const d=await r.json();
...
WebServer::HttpRequest WebServer::parse_request(int fd) {
    ...
    // Read body for POST (Content-Length)
...
void WebServer::handle_client(int client_fd) {
    ...
    if (req.method == "GET") {
        if (req.path == "/api/rooms/info") {
            ...
            else { send_response(client_fd, 400, "application/json", "{\"error\":\"invalid user_id\"}"); close(client_fd); }
            return;
        } else if (req.path == "/api/rooms/listen") {
            handle_api_rooms_listen(client_fd);
            return; // Don't close fd - SSE keeps it open
        } else if (req.path == "/admin" || req.path == "/admin/" || req.path == "/") {
            handle_admin_page(client_fd);
...
std::string WebServer::room_to_json(const std::string& room_name) const {
    ...
    std::vector<int32_t> playing_users;
    {
        ...
        // Playing users = all users minus those who have results or aborted
...
void WebServer::handle_api_rooms_listen(int fd) {
    ...
    // Keep connection open - don't close fd
    // The fd will be cleaned up when write fails
}
...
void WebServer::handle_admin_page(int fd) {
...
void WebServer::handle_admin_dissolve(int fd, const std::string& body) {
    ...
    // SSE event
    if (g_web_server) {
        // For each user that left, send leave_room event
        for (auto& u : user_list) {
...
void WebServer::handle_admin_ban(int fd, const std::string& body) {
    ...
    if (BanManager::instance().ban(uid)) {
        ...
        // Kick from room if currently connected
